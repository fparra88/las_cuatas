...
from schemas import PedidoCreate, ResumenMesa, PedidoDetalle
...
def crear_pedido(pedido: PedidoCreate, db: Session = Depends(get_db)):
    ...
    if pedido.comensal_id:
        ...
        db_pedido = Pedido(comensal_id=pedido.comensal_id, producto_id=pedido.producto_id, cantidad=pedido.cantidad, precio_unitario=producto.precio)
    else:
        ...
        db_pedido = Pedido(mesa_id=pedido.mesa_id, producto_id=pedido.producto_id, cantidad=pedido.cantidad, precio_unitario=producto.precio)
...
def obtener_pedidos(mesa_id: int, db: Session = Depends(get_db)):
    ...
    return ResumenMesa(mesa_id=mesa_id, numero_mesa=mesa.numero, pedidos=[PedidoDetalle(id=p[0].id, producto_nombre=p[1], cantidad=p[0].cantidad, precio_unitario=p[0].precio_unitario) for p in pedidos], total=total, cantidad_items=cantidad)
...
def obtener_pedidos_comensal(comensal_id: int, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "pedidos": [PedidoDetalle(id=p[0].id, producto_nombre=p[1], cantidad=p[0].cantidad, precio_unitario=p[0].precio_unitario) for p in pedidos],
        "total": total
    }
